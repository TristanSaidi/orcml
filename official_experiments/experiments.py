...
class OneDimPruningExperiment:
    ...
    def run_experiment_knn(
        ...
        while i < n_iter:
            ...
            plot_graph_2D(data, G_orc, None, edge_color=return_dict['scaled_orcs'], node_size=0.0, edge_width=0.2, colorbar=False)
            plt.savefig(f'{save_dir}/original_data_iter_{i}.png')
            plt.close()
            plot_graph_2D(data, pruned_orcml['G_pruned'], None, edge_color=pruned_orcml['preserved_scaled_orcs'], node_size=0.0, edge_width=0.2, colorbar=False)
